...
config file. This mirrors ``apps/api``'s ``shared/settings.py`` exactly (same ``AGENT_`` prefix
convention, same frozen singleton), because this service holds itself to the same configuration
discipline even though it is a separate deployable (ADR-001).

One value is deliberately *not* modelled here: ``AGENT_LLM_API_KEY``. LiteLLM reads that variable
directly from the process environment when it makes a model call (the gateway owns provider
credentials, not application code), so surfacing it through ``Settings`` would be a second,
redundant source of truth for a secret. ``llm_provider_configured`` below reports whether it is
present without this module ever reading, storing, or logging the key itself.
"""
...
class Settings(BaseSettings):
    ...
    # context — this is the same platform-wide Entra tenant, validated the same way, deliberately
    # not a second identity provider. Kept as this service's own settings (``AGENT_ENTRA_*``, not
    # imported from apps/api) because the two are separate deployables (ADR-001) that must be
    # independently configurable and independently deployable.
    ...
    # schemas, not two databases. It connects as the same least-privilege `auditmind_app` role,
    # which this service's own migration grants access to the `agent` schema.
    ...
    database_app_user: str = Field(
        ...
        "role, so RLS policies on agent.* actually apply.",
    )
    ...
    litellm_config_path: str = Field(
        ...
        "code, so a model or provider change is a config edit, not a code change.",
    )
    default_model: str = Field(
        default="primary-model",
        description="The LiteLLM model *alias* (a name from litellm.yaml's model_list, not a raw "
        "provider model id) that agent nodes route to unless a node overrides it.",
    )
    ...
    # --- Graph execution bounds ---
    max_replans: int = Field(
        default=2,
        ge=0,
        le=10,
        description="Bounded re-plan ceiling: after this many evaluation-driven re-plans, the run "
        "terminates as 'cannot conclude' rather than looping forever.",
    )
    ...
    def llm_provider_configured(self) -> bool:
        ...
        Reads ``AGENT_LLM_API_KEY`` presence only — never its value — so this can be logged and
        returned from a health endpoint safely. When ``False``, the graph still builds and every
        non-LLM node still runs; the first node that actually calls a model raises a clear, typed
        ``LlmProviderNotConfiguredError`` (see infrastructure/llm_client.py) instead of a raw
        provider authentication error.
        """
        return bool(os.environ.get("AGENT_LLM_API_KEY"))
