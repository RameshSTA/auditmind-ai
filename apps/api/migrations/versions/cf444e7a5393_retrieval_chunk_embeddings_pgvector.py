"""retrieval schema: chunk_embeddings (pgvector, Increment 08, Phase 4 §2 / Phase 6 §6-7)
...
it only ever read a column ``ingestion.chunks`` owns (Increment 07's ``search_vector``).

Schema exactly as fixed in Phase 4 §2: ``(chunk_id, model_id)`` composite primary key so a model
upgrade adds new rows rather than overwriting old embeddings (they coexist until an evaluation
harness confirms the new model is an improvement — Phase 4 §9's migration strategy; the evaluation
harness itself is Phase 9, not built yet). ``engagement_id`` is denormalized onto this table
(rather than requiring a join through ``ingestion.chunks`` to know it) for the same reason every
other table in this codebase denormalizes it: RLS policies and query filters both need it in one
hop.

Reuses the subquery-based Row-Level Security pattern from Increments 03-05 — this table has a
genuine write path (batch embedding generation) this increment exercises, the same reasoning that
pattern was chosen for every other write-path table so far.
...
# Same subquery Increments 03-05 already use — a user may act on a row if and only if they are
...
# BGE-M3's dense output dimension (Phase 4 §2 — fixed by the chosen model, not configurable).
_EMBEDDING_DIM = 1024
...
def upgrade() -> None:
    ...
    # HNSW + cosine ops, exactly as decided in Phase 4 §2 / ADR-010 — m=16, ef_construction=64 are
    # the pgvector-recommended starting values, revisited once real corpus size and query-latency
    # data exist (same doc, same section).
    op.execute(
        "CREATE INDEX chunk_embeddings_hnsw_idx ON retrieval.chunk_embeddings "
        "USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64)"
    )

    # Least-privilege grants (Phase 11 §7/§8). SELECT for search, INSERT for embedding generation
    # — no UPDATE or DELETE grant, matching this table's actual access pattern: a re-embed with an
