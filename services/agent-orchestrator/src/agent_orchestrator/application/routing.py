"""Pure routing logic for the master graph.
...
The threshold constants here are: the evaluation confidence floor below which a draft is
re-planned rather than passed forward, and the re-plan ceiling.
"""
...
#: Evaluation confidence below this routes back to the Planner for a bounded re-plan instead of
#: passing a shaky draft to Guardrail-out. A plausible default for the scaffold; the real value is
#: an evaluation-harness-calibrated number, documented as such here so it is not mistaken for a
...
def specialists_for_plan(plan: list[PlanStep]) -> list[AgentRole]:
    """The distinct evidence specialists a plan dispatches to, in a stable order.
...
def route_after_evaluation(state: AgentState, *, max_replans: int) -> str:
    """Decide what happens after the Evaluation Agent scores a draft.
    ...
    * confidence below the floor, re-plan budget remaining -> ``"replan"`` (back to the Planner
      with a broadened scope, a bounded re-plan).
    * confidence below the floor, budget exhausted -> ``"cannot_conclude"`` (an honest terminal —
      admit uncertainty rather than pass a shaky draft forward).
...
def route_after_guardrail_in(state: AgentState) -> str:
    """Decide what happens after the input Guardrail scan at entry.

    A ``violation``-severity flag from the input scan (a detected prompt-injection pattern in the
    incoming task) is a hard stop before any planning or evidence gathering — the run halts, never
    proceeding to a reasoning agent whose context the injected instruction could poison (the reason
    for scanning on *input*, not only on output). Anything else proceeds to the Planner.
    ...
    they share one append-only channel.
    """
...
def route_after_guardrail_out(state: AgentState) -> str:
    """Decide what happens after the output Guardrail scan.

    A guardrail *violation* (a hard flag in ``guardrail_flags``) is a hard stop — never retried,
    the run halts and (in a fuller build) alerts governance. Anything else proceeds to the
    human-in-the-loop interrupt. This is the 'is this safe and compliant to show' gate, kept
    strictly distinct from Evaluation's 'is this true and well-supported' gate.
    """
    flags = state.get("guardrail_flags", [])
...
def route_after_hitl(state: AgentState) -> str:
    """Decide what happens after a reviewer dispositions the interrupt.

    Approve -> Report Generation runs. Reject or edit -> the run ends without a published report,
    both the AI draft and the human decision preserved in the audit trail — this router only
    chooses the *graph* path; recording both versions is the HITL node's own write.
    """
