...
This service is a separate deployable with its own Alembic migration history, so it has its own
``Base`` rather than importing ``apps/api``'s — the two services never share a Python package.
They *do* share one physical Postgres database, and the *migration DDL* declares real
