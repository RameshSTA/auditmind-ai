...
@pytest_asyncio.fixture
async def two_engagements_with_candidates(
    admin_engine: AsyncEngine,
) -> AsyncIterator[dict[str, str]]:
    """User A is a member of engagement 1 only, which has one transaction/candidate/resolution;
    user B is a member of engagement 2 only, same shape."""
    ids = {
        ...
        "candidate_1": str(uuid.uuid4()),
        "candidate_2": str(uuid.uuid4()),
    }
    ...
    async with admin_engine.begin() as conn:
        ...
        for eng_key, txn_key, cand_key, user_key in (
            ("engagement_1", "transaction_1", "candidate_1", "user_a"),
            ("engagement_2", "transaction_2", "candidate_2", "user_b"),
        ):
            await conn.execute(
                text(
                    ...
                    "VALUES (:id, :engagement_id, 'test', 100.00, 'USD', '2026-01-01', "
                    "        '{}'::jsonb, :created_by)"
                ),
            ...
            await conn.execute(
                text(
                    "INSERT INTO kg.entity_candidates "
                    "(id, engagement_id, source_transaction_id, entity_type, raw_name, "
                    " normalized_name) "
                    "VALUES (:id, :engagement_id, :source_transaction_id, 'vendor', "
                    "        'Acme Corp', 'acme corp')"
                ),
                {
                    "id": ids[cand_key],
                    "engagement_id": ids[eng_key],
                    "source_transaction_id": ids[txn_key],
                },
            )
            await conn.execute(
                text(
                    "INSERT INTO kg.entity_resolution_map (candidate_id, neo4j_entity_id) "
                    "VALUES (:candidate_id, :neo4j_entity_id)"
                ),
                {"candidate_id": ids[cand_key], "neo4j_entity_id": str(uuid.uuid4())},
            )
...
async def test_admin_can_see_both_engagements_candidates(
    admin_engine: AsyncEngine, two_engagements_with_candidates: dict[str, str]
) -> None:
    async with admin_engine.connect() as conn:
        result = await conn.execute(text("SELECT count(*) FROM kg.entity_candidates"))
        assert result.scalar_one() == 2


async def test_app_role_sees_only_candidates_of_engagements_it_is_a_member_of(
    app_engine: AsyncEngine, two_engagements_with_candidates: dict[str, str]
) -> None:
    async with app_engine.connect() as conn:
        await conn.execute(
            text("SELECT set_config('app.current_user_id', :uid, true)"),
            {"uid": two_engagements_with_candidates["user_a"]},
        )
        result = await conn.execute(text("SELECT id FROM kg.entity_candidates"))
        rows = result.all()

    assert len(rows) == 1
    assert str(rows[0].id) == two_engagements_with_candidates["candidate_1"]


async def test_app_role_cannot_see_another_engagements_candidate_even_queried_by_id(
    app_engine: AsyncEngine, two_engagements_with_candidates: dict[str, str]
) -> None:
    async with app_engine.connect() as conn:
        await conn.execute(
        ...
        result = await conn.execute(
            text("SELECT * FROM kg.entity_candidates WHERE id = :id"),
            {"id": two_engagements_with_candidates["candidate_2"]},
        )
        assert result.all() == []


async def test_app_role_without_rls_context_sees_no_candidates(
    app_engine: AsyncEngine, two_engagements_with_candidates: dict[str, str]
) -> None:
    async with app_engine.connect() as conn:
        result = await conn.execute(text("SELECT * FROM kg.entity_candidates"))
        assert result.all() == []


async def test_app_role_sees_only_resolution_rows_of_engagements_it_is_a_member_of(
    app_engine: AsyncEngine, two_engagements_with_candidates: dict[str, str]
) -> None:
    """Proves the join-based policy independently — `entity_resolution_map` has no
    `engagement_id` column of its own, the same shape `reporting.report_findings` already
    ...
    async with app_engine.connect() as conn:
        ...
        result = await conn.execute(text("SELECT candidate_id FROM kg.entity_resolution_map"))
        rows = result.all()

    assert len(rows) == 1
    assert str(rows[0].candidate_id) == two_engagements_with_candidates["candidate_1"]


async def test_app_role_cannot_see_another_engagements_resolution_row_even_queried_by_id(
    app_engine: AsyncEngine, two_engagements_with_candidates: dict[str, str]
) -> None:
    async with app_engine.connect() as conn:
        await conn.execute(
        ...
        result = await conn.execute(
            text("SELECT * FROM kg.entity_resolution_map WHERE candidate_id = :id"),
            {"id": two_engagements_with_candidates["candidate_2"]},
        )
        assert result.all() == []


async def test_app_role_without_rls_context_sees_no_resolution_rows(
    app_engine: AsyncEngine, two_engagements_with_candidates: dict[str, str]
) -> None:
    async with app_engine.connect() as conn:
        result = await conn.execute(text("SELECT * FROM kg.entity_resolution_map"))
        assert result.all() == []
